#!/usr/bin/env python3
"""
Google Classroom Utility CLI
==============================

This single CLI tool merges the functionality for:
1.  **Assignment Auto-Solver:** Detects, solves, and submits coding assignments.
2.  **Announcement Monitor:** Fetches and summarizes course announcements.
...
5. Run:
   python merged_tool.py --help
   python merged_tool.py detect --all-courses
   python merged_tool.py summarize --all-courses --since 24
"""
...
# ---------------------- Setup ----------------------
app = typer.Typer(help="Google Classroom utility for solving assignments and summarizing announcements.")
console = Console()
load_dotenv()

# MERGED SCOPES: Includes permissions for both assignments and announcements
SCOPES = [
...
# Constants for Assignment Solver
...
class AssignmentSolver:
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                try:
                    ...
                    console.print(f"[red]Error refreshing token: {e}. Re-authenticating...[/red]")
    ...
    def _parse_timestamp(self, ts: str):
        ...
        try:
            ...
            if len(parts) == 2:
                main_ts = parts[0]
                frac_ts = parts[1].replace('+00:00', '')
                frac_ts = frac_ts.ljust(6, '0')[:6] # Truncate to 6 digits (microseconds)
                ts = f"{main_ts}.{frac_ts}+00:00"
...
"""
    ...
    def execute_code_and_capture(self, code: str, language: str, test_cases: List[Dict]) -> Tuple[str, bytes]:
        ...
        try:
            ...
            exe_file = None # Initialize exe_file
    ...
    def _create_output_screenshot(self, output_text: str, language: str) -> bytes:
        ...
        try:
            ...
            console.print("[yellow]matplotlib not found. Skipping screenshot.[/yellow]")
            return b""
    ...
    def create_solution_pdf(self, assignment_name: str, questions_data: Dict, 
        ...
        for i, (question, solution) in enumerate(zip(questions_data['questions'], solutions)):
            ...
            if 'screenshot' in solution and solution['screenshot']:
                ...
                try:
                    img = Image(io.BytesIO(solution['screenshot']), width=6*inch, height=3.6*inch, 
                              kind='proportional')
                    story.append(img)
                except Exception as e:
            ...
            if 'output' in solution:
                ...
                output_para = Preformatted(solution['output'][:1000], styles['Code']) # Use Preformatted
...
class ClassroomMonitor:
    ...
    def _setup_gemini(self):
        ...
        console.print("[green]✓ Gemini AI (Monitor) configured![/green]")
...
Format the output clearly. Do not just list the announcements.
"""
        try:
            response = self.gemini_model.generate_content(prompt)
...
def list_courses(
    ...
    # We can use either class, as both have get_courses()
...
def detect(
    ...
    for course in courses:
        ...
        if not coursework:
            console.print("[dim]No assignments found in time window.[/dim]\n")
            continue
        ...
        for assignment in coding_assignments:
            ...
            with Progress(
                ...
                for i, question in enumerate(questions_data['questions'], 1):
                    ...
                    # Generate solution
                    solution_task = progress.add_task(f"Generating solution {i}...", total=None)
                    solution = solver.generate_solution(question, questions_data['language'])
                    progress.update(solution_task, description=f"✓ Solution {i} generated")
                    
                    # Execute and capture
                    exec_task = progress.add_task(f"Executing code {i}...", total=None)
                    output, screenshot = solver.execute_code_and_capture(
                        solution['code'],
                        questions_data['language'],
                        solution.get('test_cases', [])
                    )
                    progress.update(exec_task, description=f"✓ Code {i} executed")
                    progress.stop()
                ...
                # Create PDF
                pdf_task = progress.add_task("📄 Generating solution PDF...", total=None)
                ...
                progress.update(pdf_task, description=f"✓ PDF saved: {pdf_path}")
                progress.stop()
                ...
                # Submit if requested
                if auto_submit:
                    submit_task = progress.add_task("📤 Uploading and submitting...", total=None)
                    ...
                    if drive_file_id:
                        ...
                        total_submitted += 1
                        progress.update(submit_task, description="[bold green]✅ Assignment submitted successfully![/bold green]")
                    else:
                        progress.update(submit_task, description="[red]Failed to upload file.[/red]")
                    progress.stop()
                else:
...
def summarize(
    ...
    for course in courses:
        ...
        if not anns:
            console.print("No announcements found in time window.\n")
            continue
        ...
        for i, ann in enumerate(anns, start=1):
            ...
            text = ann.get('text', 'No content').strip().replace('\n', ' ')[:150] + "..."
            console.print(f"{i}. [{timestamp}] {text}")
